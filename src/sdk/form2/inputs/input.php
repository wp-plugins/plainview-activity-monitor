...
class input
{
	use \plainview\sdk_pvam\html\element;
	use \plainview\sdk_pvam\html\indentation;
	...
	**/
	...
	{
		$random = \plainview\sdk_pvam\base::uuid();
	...
	**/
	public function display_input()
	{
		...
		if ( $input->form()->post_is_set() )
		{
			...
			if ( $value != '' )
			{
				$value = \plainview\sdk_pvam\form2\form::unfilter_text( $value );
				$input->value( $value );
			}
			else
	...
	**/
	...
	{
		$r = new \plainview\sdk_pvam\html\div();
	...
	**/
	public function make_id()
	{
		if ( $this->has_attribute( 'id' ) )
			return $this->get_attribute( 'id' );
		$id = $this->make_name();
		$id = \plainview\sdk_pvam\base::strtolower( $id );
	...
	**/
	...
	{
		$title = \plainview\sdk_pvam\form2\form::filter_text( $title );
